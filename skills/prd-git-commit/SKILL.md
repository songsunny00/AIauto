...
| -------------- | ----------------- | ------------ | ---------------------- | ------------ | ------------------------- |
| `{YYYY-MM-DD}` | `{模块编码/名称}` | `{变更说明}` | `{功能点}｜{使用场景}` | `{REQ-xxx}`  | `变更人: {git user.name}` |
...
| 变更日期       | 关联变更  | 使用注意         | 当前限制         |
| -------------- | --------- | ---------------- | ---------------- |
| `{YYYY-MM-DD}` | `#{序号}` | `{后续注意事项}` | `{当前限制说明}` |

- 「关联变更」列填写本次变更在第 3 节「详细变更项」中对应的序号（如 `#7`；同批次多变更用 `#7-8`），不再重复填写「影响模块」（已在第 2 节「模块」列体现）。
