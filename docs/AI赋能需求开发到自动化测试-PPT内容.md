...
**对应解决方式**——一句话原理：**统一模板结构化的需求文档 + 增量维护全量文档**。两个动作做到：

1. **首次建基线**（模块第一次接入时）：把产品原始输出整理成结构化、模板统一的 `01-需求文档.md`，每个功能点打上唯一的 `REQ` 编号。这份文档就是这个模块的**需求基线**——之后开发、测试、AI 都从它取信息，不再依赖零散的原始材料。

2. **每版增量回写**：版本关闭时用 `version-archive` 把本版的新增/修改按 REQ 合并回基线，让基线始终是最新最全的一份。

对应到仓库的物理形态——**版本目录存增量、模块目录存全量、版本关闭时增量合并进全量**：

```text
Prds/
├─ CONFIG-配置中心/BILLCFG-商业化计费配置/
│  └─ 01-需求文档.md          ← 全量基线（始终最新最全）★ 开发/AI 只看这份
└─ V1.6.3/
   └─ 01-需求文档.md          ← 本版增量（只写变动点）
```

> 查任何功能点的**完整描述只看基线这一处**，不用翻历次版本文档拼凑。
...
| **REQ 统一主键** | 编号自带模块信息，一眼看出功能点归属，贯穿需求/设计/开发/测试/缺陷/回归                 |
| **结构化目录**   | `Prds/` 目录 + 文件名 + 编号三位一体，**天然支持检索**，解决 Ones 不支持 RAG 问答的问题 |

**REQ 编号自带模块信息**——拿到编号就知道它属于哪个模块、哪个功能区域：

```text
REQ-CONFIG-BILLCFG-LIST-001
 │       │       │   │    └─ 功能点序号（第 001 号）
 │       │       │   └─ 功能区域：LIST（列表）
 │       │       └─ 二级模块：BILLCFG（商业化计费配置）
 │       └─ 一级模块：CONFIG（配置中心）
 └─ 前缀：REQ
```

> 从脚本名、缺陷单、测试报告里看到 `REQ-CONFIG-BILLCFG-...`，不用查表就知道是配置中心的活——讨论、检索、影响面判断都省一轮沟通。
...
- **新一代浏览器自动化框架（E2E），开箱即用**：微软 2020 年发布，原生支持 Chromium / Firefox / WebKit 三浏览器，一套 API 全覆盖，不需要为多浏览器写多套脚本
- **自动等待，天生稳**：操作前自动检查元素状态——可见、尺寸稳定、可交互才执行，不用手写 sleep 轮询，flaky 用例天然就少
- **失败可回放**：失败自动留存 trace（每步操作 + 页面快照 + 网络请求），用 Trace Viewer 逐步回放定位问题，不用反复复跑猜原因
- **UI 与 API 同栈**：UI 操作和接口断言可写进同一条用例（共享登录态），一套 runner、一套断言体系；还能拦截 / mock 接口——实战里用它验证前端缓存复用
...
- 优化/新增 7 个 skill 规则（4 类越界拦截 + 2 条详设规则 + 1 条需求检测维度）
...
| 有效 bug | 4 项（DEFECT：授权状态文案、合同编号查询、缓存复用、编辑保存反馈） |
| 误报     | 3 项（SCRIPT 断言策略：颜色、toast 范围、未逐列检查）              |
| 漏测     | 8 项（脚本不完善 3 + 用例不完善 2 + 需求不完善/错误 3）            |
| 跳过     | 4 项（存储合同、分页、启停，均为测试数据不足，属隐性未覆盖）       |
| 错误     | 2 项（SCRIPT：定位回归、子表行不足）                               |

> 关键结论：通过率 80.0% ≠ 质量——§4.3 的 8 项漏测 + 5 类未覆盖盲区 + 4 个 skip 都是通过率无法反映的，这也是 33 个缺陷中 AI 仅检出 4 个的核心原因；漏测中 37.5% 源于需求本身缺失或错误（子表排序规则、业务限制逻辑、明细页过滤口径），需在 01 评审阶段补齐。
